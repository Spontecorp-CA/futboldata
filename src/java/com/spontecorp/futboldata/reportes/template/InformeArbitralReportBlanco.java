 ...
 * @author sponte03
 */
public class InformeArbitralReportBlanco {

    private static FieldBuilder<?> titularField;
    private static Partido partido;

    public static JasperReportBuilder crearReporte(Partido partido, List<EquipoHasJugador> equipoLocal,
            List<EquipoHasJugador> equipoVisitante) {


        InformeArbitralReportBlanco.partido = partido;
        ...
        SubreportBuilder subreportStaffVisitante = reporteStaff("Cuerpo Técnico Visitante").setDataSource(createDataStaff(null));
        SubreportBuilder subreportStaffLocal = reporteStaff("Cuerpo Técnico Local").setDataSource(createDataStaff(null));

        SubreportBuilder subreportArbitro = reporteArbitro().setDataSource(createDataArbitro(null));

        SubreportBuilder subreportCambioL = reporteCambios("Equipo Local").setDataSource(createDataCambio(null));
        SubreportBuilder subreportCambioSalidaL = reporteCambiosSalida("").setDataSource(createDataCambioSalida(null));

        SubreportBuilder subreportCambioV = reporteCambios("Equipo Visitante").setDataSource(createDataCambio(null));
        SubreportBuilder subreportCambioSalidaV = reporteCambiosSalida("").setDataSource(createDataCambioSalida(null));

        SubreportBuilder subreportTarjetaL = reporteTarjetas("Tarjeta Equipo Local").setDataSource(createDataTarjeta(null));
        SubreportBuilder subreportTarjetaV = reporteTarjetas("Tarjeta Equipo Visitante").setDataSource(createDataTarjeta((null)));

        SubreportBuilder subreportGolesL = reporteGoles("Goles Equipo Local").setDataSource(createDataGoles(null));
        SubreportBuilder subreportGolesV = reporteGoles("Goles Equipo Visitante").setDataSource(createDataGoles(null));
    ...
    private static JRDataSource createDataEquipo(List<EquipoHasJugador> equipo) {
        ...
        size = 35 - size;
        for (EquipoHasJugador equi : equipo) {
